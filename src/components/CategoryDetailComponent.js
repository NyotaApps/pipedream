...
class CategoryDetailComponent extends Component{
      ...
      if(this.props.similarProductData.getCurrentToken.currentToken=='-1'){
            ...
            if((categoryData.wishListID!==''||this.state.wID!=0)){
                if(this.state.wID==-1){
                    ...
                    .then((response) => {
                           //
                           if(this.props.navigation.state.params.productData.wishListID!==''){
                             this.setState({
                               wID:-1
                             })
                           }else{
  ...
  //
  //   let requestData={
  //     "category_id":this.props.navigation.state.params.subCategoryId,
  //     "page_no":'1'
  //   };
  ...
  //   client.post('http://nyota-app.com/nyotastaging/api/categorydetail',requestData)
  //   .then((response) => {
  ...
  callAPIfun=()=>{
    ...
    this.props.postRequest(SIMILAR_PRODUCT,requestData);
      ...
      render(){
        //
        if(this.props.similarProductData.apiData.isFetchning===false
          && this.props.similarProductData.apiData.data.data!==undefined){
              isAvailable = true;
          }
        ...
        return(
          <View style={{flexGrow:1}}>
                <NavigationEvents
                  ...
                  onDidBlur={payload => console.log('CatCom will focus', payload)}
                 />
                ...
                <SafeAreaView style={{flexGrow:1}}>
                        ...
                        <ScrollView style={{flexBasis: 0,width: Dimensions.get('window').width}} nestedScrollEnabled={true}>
                            <this.ImageViewContainer/>
                              ...
                              {(this.props.navigation.state.params.productData.attribute && this.props.navigation.state.params.productData.attribute.constructor === Array && this.props.navigation.state.params.productData.attribute.length === 0)
                                ...
                                null
                                :
                                <this.setAttributeData attributeData={this.props.navigation.state.params.productData.attribute[this.state.quantitySelectedPos]}/>
                              }
                             ...
                             <View style={{marginTop: 10,backgroundColor: 'white',height:undefined,flexDirection: 'column',paddingBottom: 10}}>
                                  <View  style={{flexDirection:'row',justifyContent:'space-between'}}>
                                      ...
                                      <TouchableOpacity onPress={()=>{
                                                  navigate('SimilarPCC',{
                                                    ...
                                                    cName:stringsoflanguages.similarProductText
                                                  })}}>
                                              {(this.props.similarProductData.apiData.data.data!==undefined &&
                                                this.props.similarProductData.apiData.data.data.length>1)?
                                  ...
                                  <FlatList
                                        ...
                                        keyExtractor={(item, index) => index.toString()}
                                        renderItem={ ({item}) =>
                                         <SimilarProductRow itemData={item}
                                                            sProductNavigate={navigate}
                                                          languageCode={(this.props.similarProductData.getLanguageCode.languageCode=='en')?'en':'fr'}
                                                          userToken={this.props.similarProductData.getCurrentToken.currentToken}
                    ...
                    <Footer footerNavigate={navigate} screenPos={2} footerCartCounter={Number(this.props.similarProductData.getCartCounter.cartCounter)}/>
                </SafeAreaView>
          </View>
        );
      }
                 ...
                 <View style={{width: Dimensions.get('window').width,flexDirection: 'column',backgroundColor: 'white'}}>
                             ...
                             <View style={{position: 'absolute',width: Dimensions.get('window').width-40,
                                           ...
                                           <TouchableOpacity onPress={()=>this.onClickWishListCalledID(this.props.navigation.state.params.productData)}>
                                                   <Image source={((this.props.navigation.state.params.productData.wishListID!==''||this.state.wID!=0)
                                                     &&(this.props.navigation.state.params.wishStatus==0||this.props.navigation.state.params.wishStatus==1))?(this.state.wID==-1)?require('../../images/profile/wishlist.png'):require('../../images/profile/wishlist_dark.png'):require('../../images/profile/wishlist.png')}
                      ...
                      <View style={{padding:20,flexDirection: 'row'}}>
                             <View style={{flexDirection: 'column',flexGrow: 3,flexBasis: 0}}>
                                   ...
                                   {(this.props.navigation.state.params.productData.attribute && this.props.navigation.state.params.productData.attribute.constructor === Array && this.props.navigation.state.params.productData.attribute.length === 0)?null:
                             ...
                             {(this.state.attributeQtyCounter && this.state.attributeQtyCounter.constructor === Array && this.state.attributeQtyCounter.length === 0)
                               ?
      ...
      onClickAddCart=()=>{
        ...
        if(this.props.similarProductData.getCurrentToken.currentToken=='-1'){
          ...
          //commented below line at 14th Sep 2020
      ...
      CheckOnBackPressEvent=()=>{
        ...
        let requestData={
          ...
          "page_no":Math.ceil(Number(this.props.navigation.state.params.indexPos)/25).toString(),
              ...
              for(var i=0;i<response.data.data.length;i++){
                if(response.data.data[i].product_id==this.props.navigation.state.params.productData.product_id){
                    ...
                    if(typeof(response.data.data[i].cart)=='string'){
                       ...
                       for(var j=0;j<response.data.data[i].attribute.length;j++){
                          //
                         if(response.data.data[i].cart.some(cart => cart.sale_price === response.data.data[i].attribute[j].sale_price)){
                              this.state.attributeQtyCounter[j].quantity = response.data.data[i].cart[response.data.data[i].cart.findIndex(cart => cart.sale_price == response.data.data[i].attribute[j].sale_price)].quantity
...
export default connect(mapStateToProps,{postRequest,reset,setToken,setCartCounter,setNotificationCounter,setLanguageCode})(CategoryDetailComponent);
